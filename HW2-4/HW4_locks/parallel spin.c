/*Answear for Part 2:
For 100000 insertion/retrieval:
4 threads:	spinlock 0.013s	mutex 0.014s
10 threads:	spinlock 0.017s	mutex 0.015s
100 threads:	spinlock 0.013s	mutex 0.016s
1000 threads:	spinlock 0.023s	mutex 0.028s	

Spinlock seem to be faster than mutex. I thought spin lock would be slower because 
busy waiting occupy 100% cpu time. In this case spinlock is faster
probably because the critical region is only 2 lines of C code, which means 
threads don't have to wait for too long for the thread in critical region 
to finish and release the lock. For mutex, any time a thread unable to acquire 
lock will have to make a system call whcih is pretty expansive. The mutex have to
manage a long list of threads to wake up, adding to more overhead.

Spinlock does seem to have a bigger variance in terms of run time compare to mutex. 
Mutex run times are quite consistent while spinlock occationally increase to several
time the average value. It is possible that in those runs thread in critial region is
accidentally preempted. The scheduler switch to other threads which keep spinning and
use up their quanta.    
*/
#include <stdio.h>
#include <stdlib.h>
